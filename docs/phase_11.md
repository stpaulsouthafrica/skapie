...
3. Prove the native write path before designing the full Apply UI. The existing Repository grant is read-only. Introduce a separate, user-selected write scope and visible grant; do not treat the stored path or read bookmark as write authority. Keep an option to export a proposal when write access is unavailable.
   ...
   // implemented and release-built; native picker/write acceptance remains hands-on
...
4. Preview a file-level diff and allow Accept or Reject. Bind the decision to the exact proposal ID and content fingerprint; editing a proposal invalidates its earlier acceptance. Accept creates a review decision, but the user still invokes Apply explicitly in this phase. A model's call to Propose cannot approve or apply its own proposal. A later bounded policy may change that, visibly. Auto-apply / YOLO is out of scope for this phase and must be a later visible kit on the same machinery.
   ...
   // done; decisions settle until explicit Reconsider or a new proposal
...
5. Check path containment, current file fingerprints, and patch applicability immediately before application. Git's [git apply --check](https://git-scm.com/docs/git-apply) can inform the implementation, but the app still validates authority and path scope itself. A stale file produces a reviewable conflict, not an overwrite.
   ...
   // implemented with host checks; hands-on macOS acceptance pending
...
11.4.1. Resolve the macOS execution architecture first. A user-selected read bookmark does not authorize running executables in that folder; Apple's [App Sandbox guidance](https://developer.apple.com/documentation/security/accessing-files-from-the-macos-app-sandbox) treats executable access separately. Test the smallest viable signed and scoped execution path for the intended distribution. Document the choice and its limits; do not prescribe a helper process unless the spike shows it is necessary, and never silently disable the app sandbox.
   - **What this is.** First decide how a sandboxed Skapie build is allowed to start a process at all. Folder read access is not run authority. Prefer the smallest honest path; only add a helper if the spike proves you need one; never turn the sandbox off quietly.
   - **How to test.** On a release-signed sandboxed build, attempt the smallest intended check. Record what Apple allows, what fails, and why. Confirm a repository read bookmark alone cannot start an executable. Confirm the sandbox entitlement stays on. If a helper is proposed, document the necessity with a failed direct path; otherwise keep the direct path.

11.4.2. Add separate **Check Spec**, **Run Check**, and **Check Result** kits. For the first slice, the user configures one exact executable plus argument list or chooses one trusted predefined check. The model may inspect the result but cannot supply a free-form shell command. Connecting a cable never starts a process.
   - **What this is.** Three kits: Spec holds the exact command you chose; Run is the explicit start effect; Result holds the outcome. The model can read Result. It cannot invent a shell string. Wiring alone never executes.
   - **How to test.** Place Spec, Run, and Result, cable them, and confirm no process starts. Configure one exact executable + args (or one trusted predefined check), start Run, and confirm Result fills. Attempt a free-form shell command from the model path; it must be refused. Confirm disconnected Run cannot execute from Spec alone.

11.4.3. Treat a check as an **effectful execution grant**: build and test commands can write generated files or even source files. Prefer an isolated copy or scoped output directory when feasible; otherwise require explicit repository write scope and record before/after Git state. Execute with exact executable and arguments, minimal environment, a validated working directory, timeout, bounded output, and process-tree cancellation. A contained working directory alone does **not** confine what the process can read, write, or contact; claim isolation only after the chosen OS mechanism proves it. Decide whether network access is permitted and show the decision. Expand to configurable command catalogs only after this path works.
   - **What this is.** Running a check is a grant with side effects, not a read. Prefer isolation; if the process can touch the repo, require write scope and record Git before/after. Bound argv, env, cwd, timeout, output, and cancel the process tree. Do not claim sandbox isolation you have not proven. Show network allow/deny. No command catalog until this one path is solid.
   - **How to test.** Run a check that writes under a scoped output dir and confirm only that scope changes. Without write scope, confirm a repo-touching check is refused or forced into the isolated path you chose. Confirm timeout kills or clearly reports uncertain stop. Cancel mid-run and confirm the tree stops or uncertainty is reported. Confirm UI states whether network is allowed. Confirm cwd alone is never marketed as full isolation.

11.4.4. Capture command argv, cwd, environment key names, start/end time, exit code, stdout/stderr chunks, truncation, cancellation, and resulting Git state. Redact secrets in display and saved output. Stream chunks into the same run ledger; do not attach an unrelated terminal transcript to the LLM card.
   - **What this is.** Every run leaves an attributable record: what ran, where, env keys (not secret values), timing, exit, streamed output, truncation/cancel flags, and Git delta if recorded. Secrets are redacted. Evidence lives on the check/run ledger, not pasted onto the LLM card.
   - **How to test.** Run a check that prints a fake secret-shaped string and confirm display/saved output redact it. Open the run ledger and confirm argv, cwd, env key names, times, exit, chunks, and Git state are present. Confirm the LLM card does not grow an unrelated terminal dump. Truncate a large stdout and confirm truncation is marked.

   // implemented and automatically tested; hands-on macOS acceptance pending

11.4.5. Start with the honest result categories **exit 0**, **nonzero exit**, **timeout**, **cancelled**, and **infrastructure error**. A test-specific pass/fail parser is a later adapter; do not infer that every command's exit 0 means the entire task is correct.
   - **What this is.** Five honest outcomes only. Exit 0 means the process exited 0, not “the whole agent task succeeded.” Richer test parsers come later as adapters.
   - **How to test.** Force each category once (true exit 0, failing exit, short timeout, user cancel, broken executable path). Confirm UI labels match and none of timeout/cancel/infra are shown as pass. Confirm an exit 0 check does not auto-mark the broader task complete.

   // implemented and automatically tested; hands-on macOS acceptance pending

11.4.6. Offer Repeat Check only as another explicit user action with fresh execution authority. A replayed trace never reruns the process.
