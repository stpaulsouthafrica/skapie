...
- **Double-click** a text kit: opens it in the **Full Screen text editor**. Closing or Esc saves through KitApi. The magnifying glass on a run evidence row, and full screen on Request Information, open the same editor read-only.
- **Full Screen text editor**: line numbers down the left. Hover and the caret light only that line's number. The text itself stays clear. The header shows the detected language (JSON, Dart, or Text), a Read only chip when it can't be edited, and the caret line and column. JSON and Dart are colored automatically from the content.
